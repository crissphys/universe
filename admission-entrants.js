(function(){
  ...
  function setStats(){id('entrantApplicants').textContent=nf0.format(data.stats.ordinaryApplicants);id('entrantCount').textContent=nf0.format(data.stats.ordinaryEntrants);id('entrantFaculties').textContent=nf0.format(data.stats.faculties);id('entrantCareers').textContent=nf0.format(data.stats.careers)}
  function confetti(){
  ...
  function celebrate(row){
    ...
    id('entrantMessage').innerHTML='Hoy celebramos una meta construida con constancia, disciplina y muchas horas de preparación. <strong>'+safe(row.name)+'</strong>, tu ingreso a <strong>'+safe(row.career)+'</strong> reconoce todo ese esfuerzo y abre una nueva etapa llena de retos, aprendizaje y oportunidades.<br><br>Que este resultado sea el comienzo de una trayectoria universitaria que te permita crecer, aportar y transformar tu entorno. Disfruta este momento junto a las personas que te acompañaron: <strong>bienvenido a la Universidad Nacional de Ingeniería</strong>.';
  ...
  function renderMatches(matches){var box=id('entrantMatches');if(!matches.length){box.innerHTML='<div class="entrants-match"><span><b>No encontramos coincidencias</b><small>Revisa el nombre o código e inténtalo otra vez.</small></span></div>';return}if(matches.length===1){box.innerHTML='';celebrate(matches[0]);return}box.innerHTML=matches.map(function(row){return'<button type="button" class="entrants-match" data-code="'+safe(row.code)+'"><span><b>'+safe(row.name)+'</b><small>'+safe(row.code)+' · '+safe(row.career)+'</small></span><strong>'+points(row.score)+'</strong></button>'}).join('');box.querySelectorAll('[data-code]').forEach(function(button){button.onclick=function(){var row=data.rows.find(function(item){return item.code===button.dataset.code});if(row){box.innerHTML='';celebrate(row)}}})}
  function renderList(){
    var q=norm(id('entrantListSearch').value),faculty=id('entrantFacultyFilter').value;
    var rows=data.rows.filter(function(row){return(!faculty||row.facultyCode===faculty)&&(!q||norm(row.code+' '+row.name+' '+row.career).indexOf(q)>=0)});
  ...
  function init(){
    ...
    id('entrantListSearch').addEventListener('input',renderList);select.addEventListener('change',renderList);renderList();
  }
