...
const routes={
 ...
 '/admision':['Tu futuro toma forma.','ADMISIÓN / UNI','Conoce el proceso, explora tus opciones y conecta tu preparación con información verificable.'],
 '/simulacros':['Practicar con propósito.','LABORATORIO / EVALUACIÓN','Elige tu enfoque, revisa qué estudiar y continúa al simulador conectado cuando quieras evaluar tu preparación.'],
 '/examenes':['Aprende de cada examen.','ARCHIVO / CEPREUNI','Conecta evaluaciones, temas y materiales. El archivo de esta propuesta distingue los recursos disponibles de los servicios conectados.'],
 ...
 '/unitalk':['El conocimiento se comparte.','COMUNIDAD / UNITALK','Encuentra tu comunidad y sigue conectado con Universe. Las conversaciones reales permanecen en el servicio original.'],
 ...
 '/ranking':['Tu avance tiene contexto.','COMUNIDAD / RANKING','El diseño del seguimiento, sin inventar usuarios ni puntajes. Tus resultados reales se consultan en la plataforma conectada.'],
 '/docentes-cepreuni':['Encuentra tu próximo salón.','CEPREUNI / GUÍA DE AULAS','Accede a docentes, salones y horarios del ciclo. La información vigente se mantiene en la guía original.'],
 '/informacion-cepreuni':['El ciclo, más claro.','CEPREUNI / INFORMACIÓN','Documentos, evaluaciones y recursos reunidos para orientarte durante tu preparación.'],
 '/nosotros':['El origen es la curiosidad.','ACERCA DE UNIVERSE','Una comunidad educativa independiente que reúne recursos para postulantes UNI y estudiantes CEPREUNI.'],
 '/metodologia-editorial':['Claridad desde la fuente.','UNIVERSE / METODOLOGÍA','La confianza empieza por distinguir el material, su procedencia, su disponibilidad y sus límites.'],
 '/correcciones':['Mejoramos contigo.','UNIVERSE / CORRECCIONES','Si encuentras una errata, un enlace roto o información desactualizada, ayúdanos a ubicarla con precisión.'],
 '/contacto':['Hablemos de tu siguiente paso.','UNIVERSE / CONTACTO','Los canales originales de Universe, reunidos en un mismo espacio.'],
 '/terminos':['Las reglas, a la vista.','INFORMACIÓN / TÉRMINOS','Esta propuesta visual no modifica las condiciones de uso de Universe. Consulta siempre la versión publicada.'],
 '/privacidad':['Tu información importa.','INFORMACIÓN / PRIVACIDAD','Conoce qué permanece en tu navegador en esta demostración y cuándo estás saliendo hacia otro servicio.'],
};
...
function sidebar(path){const items=[['/explorar','Plataforma','grid'],['/cepreuni','CEPREUNI','cap'],['/biblioteca','Biblioteca','book'],['/temario','Temario','list'],['/clases','Clases','play'],['/planificador','Planificador','calendar'],['/simulacros','Simulacros','target'],['/examenes','Exámenes','layers'],['/admision','Admisión UNI','cap'],['/calculadora','Calculadora','calculator'],['/unitalk','UNITALK','users'],['/ranking','Ranking','orbit'],['/docentes-cepreuni','Guía de aulas','grid']];return `<span>TODO CONECTA</span><nav aria-label="Secciones de Universe">${items.map(([url,t,i])=>`<a href="${url}" ${path===url||(url==='/biblioteca'&&path.startsWith('/biblioteca/'))?'aria-current="page"':''}>${ui.icon(i)}${t}</a>`).join('')}</nav><div class="rail-foot"><b>UNIVERSE / GALAXY</b><br>Una propuesta local.<br>Tu sitio publicado sigue intacto.</div>`}
...
async function route(focus=false){const token=++routeVersion,p=pathName();const isHome=p==='/';home.hidden=!isHome;$('#platform').hidden=isHome;document.body.classList.toggle('in-platform',!isHome);$('.brand').href='/';$('.footer-brand').href='/';$$('.header .nav a').forEach(a=>a.setAttribute('aria-current',a.getAttribute('href')===p?'page':'false'));if(isHome){document.title='Universe — El origen de tu siguiente idea';return}document.title=`${routes[p]?.[0]||'Página no encontrada'} — Universe`;$('.side-rail').innerHTML=sidebar(p);view.innerHTML=head(p)+'<div class="page-view" id="page-view"></div>';const out=$('#page-view');if(!data){out.innerHTML=empty('Conectando tus recursos…');try{data=await fetch('/universe-ui/data/platform.json').then(r=>{if(!r.ok)throw Error('Datos no disponibles');return r.json()});hydrateHumanities(data)}catch{out.innerHTML=note('No se pudieron cargar los recursos. Recarga esta página.');return}}if(token!==routeVersion)return;
 ...
 else if(p==='/cepreuni'||p==='/informacion-cepreuni')cepre(out);
 ...
 else if(p==='/admision')admissions(out);
 else if(p==='/examenes'||p==='/simulacros')practice(out,p);
 else if(p==='/account')account(out);
 else if(p==='/unitalk'||p==='/contacto')community(out,p);
 else if(p==='/ranking'||p==='/docentes-cepreuni')connected(out,p);
 else if(routes[p])information(out,p);
...
function buildPlan(profile){
 ...
 for(let d=0;d<weeks*7;d++){const date=new Date(startDate);date.setDate(date.getDate()+d);const weekday=date.getDay();if(weekday===0||!days.includes(weekday))continue;
  for(let b=0;b<blocksPerDay;b++){const areaNames=AREAS.map(([n])=>n).sort((a,c)=>counts[a]/weights[a]-counts[c]/weights[c]);const area=areaNames[0];counts[area]++;const pick=nextForArea(area);const iso=date.toISOString().slice(0,10);entries.push({date:iso,index:b,...pick,done:false,id:iso+'-'+b})}}
 return entries;
}
...
function admissions(out){out.innerHTML=`<section class="p-panel p-feature"><div><span class="p-label">UNIVERSIDAD NACIONAL DE INGENIERÍA</span><h2>Una meta grande.<br>Decisiones informadas.</h2><p>Organiza tu preparación sin confundir una referencia histórica con un resultado asegurado.</p><div class="page-actions">${link('https://admision.uni.edu.pe/','Consultar Admisión UNI','button primary',true)}</div></div>${orbit()}</section>${sub('Conecta tu preparación')}<div class="p-grid">${tile('/temario','Temario de admisión','Revisa el alcance completo por curso.','list')}${tile('/planificador','Plan de preparación','Organiza temas y sesiones.','calendar')}${tile('/simulacros','Práctica y evaluación','Explora tus siguientes desafíos.','target')}</div>${sub('Información del proceso')}<div class="p-panel">${link('/admision','Carreras, referencias de corte y análisis de admisión','link-line',true)}${link('/resultados-admision','Resultados de admisión','link-line',true)}${link('https://admision.uni.edu.pe/','Cronograma, requisitos y comunicados oficiales','link-line',true)}</div>${note('No se muestran puntajes, vacantes o fechas de admisión inventadas. La información que cambia con el proceso se consulta en su fuente vigente.')}`}
function practice(out,p){out.innerHTML=`<div class="p-split"><section class="p-panel"><span class="p-label">PREPARA TU EVALUACIÓN</span><h2 style="margin-top:16px">Elige un punto de partida.</h2><label class="p-label" for="practice-eval">Evaluación</label><select id="practice-eval" class="p-select" style="display:block;width:100%;margin-top:12px">${data.syllabus.evaluations.map((e,i)=>`<option value="${i}">${esc(e.name)}</option>`).join('')}</select><div id="practice-scope"></div><div class="page-actions">${internal('/temario','Explorar estos cursos','button primary')}${internal('/fijas-cepreuni','Fijas Primera PC','text-button')}</div></section><aside class="p-panel"><span class="p-label">SERVICIO CONECTADO</span><h2 style="margin-top:17px">Practica en el motor original.</h2><p>La generación con IA, el banco privado y la calificación requieren el servicio real. Esta vista de diseño no realiza llamadas ni consume créditos.</p>${link('https://universetostudy.com'+p,p==='/simulacros'?'Abrir simulador original':'Abrir archivo original','button',true)}<p class="source-note">Al continuar, sales de la vista previa local.</p></aside></div>${sub('Todo listo para prepararte')}<div class="p-grid">${tile('/cepreuni/ciclopre20271','Material del ciclo','Abre los documentos y libros disponibles.','layers')}${tile('/clases','Revisar una explicación','Encuentra videos del curso.','play')}${tile('/planificador','Organizar el repaso','Lleva tus temas a tu planificador.','calendar')}</div>`;const render=()=>{const e=data.syllabus.evaluations[Number($('#practice-eval').value)];$('#practice-scope').innerHTML=`<p class="p-notice">${e.range?'Semanas '+e.range.join(' a '):'Alcance del temario'}</p><div class="p-chips">${(e.subjects||[]).map(s=>`<span class="p-chip">${esc(s)}</span>`).join('')}</div>`};$('#practice-eval').onchange=render;render()}
function account(out){renderProfile(out,data,ui.openPreferences)}
function community(out,p){const channels=[['https://whatsapp.com/channel/0029VbBKG4SAojYveJl8MJ0T','WhatsApp','Avisos y novedades de la comunidad.','message'],['https://www.youtube.com/@universepre_official','YouTube','Contenido del canal de Universe.','play'],['https://www.tiktok.com/@universe_preuni','TikTok','Universe en formato breve.','play'],['https://linktr.ee/universetostudy','Linktree','Todos los enlaces, reunidos.','grid']];out.innerHTML=`<section class="p-panel p-feature"><div><span class="p-label">UNA COMUNIDAD, MUCHAS CONEXIONES</span><h2>Tu curiosidad<br>no está sola.</h2><p>Conecta con los canales de Universe. Sin conversaciones, mensajes ni usuarios ficticios en la demostración.</p><div class="page-actions">${link('/unitalk','Entrar a UNITALK','button primary',true)}</div></div>${orbit()}</section>${sub('Nuestros canales')}${brandRow(data.logos)}<div class="p-grid two">${channels.map(([u,t,d,i])=>tile(u,t,d,i,'Abrir canal original',true)).join('')}</div>${sub('¿Necesitas ayuda?')}<div class="p-panel">${link('https://wa.me/51963385410?text=Hola%2C%20necesito%20ayuda%20con%20Universe%20to%20Study','Contactar con soporte de Universe','link-line',true)}</div>`}
function connected(out,p){const isRank=p==='/ranking';out.innerHTML=`<div class="p-split"><section class="p-panel"><span class="p-label">${isRank?'RESULTADOS CON IDENTIDAD':'ORGANIZACIÓN DEL CICLO'}</span><h2 style="margin-top:17px">${isRank?'Lo importante es tu progreso.':'Un lugar para cada clase.'}</h2><p>${isRank?'Consulta el ranking y tus resultados reales en el servicio conectado. No se presentan posiciones ni estudiantes de ejemplo como si fueran datos reales.':'La guía de docentes, salones y horarios necesita los datos vigentes del ciclo. Para no mostrar información desactualizada, la consulta se realiza en su fuente original.'}</p><div class="page-actions">${link('https://universetostudy.com'+p,isRank?'Consultar ranking real':'Abrir guía de aulas','button primary',true)}</div>${note('Esta pantalla muestra el nuevo formato. La cuenta y los datos en vivo se mantienen en la plataforma original.')}</section><aside class="p-panel">${orbit()}<span class="p-label">EL SIGUIENTE PASO DEPENDE DE TI</span>${internal(isRank?'/planificador':'/cepreuni',isRank?'Organizar mi avance':'Volver a mi ciclo','link-line')}</aside></div>${sub('Sigue conectado')}<div class="p-grid two">${tile('/temario','Revisar mis conceptos','Continúa tu preparación con el temario.','list')}${tile('/cepreuni/ciclopre20271','Materiales del ciclo','Recursos para tu próxima sesión.','layers')}</div>`}
function information(out,p){const content={
 '/nosotros':[['Aprender, practicar, conectar.','Universe reúne herramientas de preparación, recursos y comunidad para estudiantes que quieren avanzar. El espacio conecta temarios, clases, bibliotecas, planificación y evaluaciones.'],['Una comunidad independiente.','Universe no se presenta como una página oficial de la UNI o de CEPREUNI. Los enlaces institucionales se diferencian de los materiales y herramientas de la comunidad.']],
 '/metodologia-editorial':[['La fuente permanece visible.','Los libros mantienen su portada, título, autor y enlace de origen. Los materiales próximos no se muestran como si estuvieran disponibles.'],['No todo dato es una garantía.','Las fijas se identifican como un modelo histórico. Las fechas requieren confirmar cambios con la institución y los resultados en vivo se consultan en la plataforma conectada.']],
 '/correcciones':[['Cuéntanos dónde ocurre.','Indica el curso, título del material, página o enlace y describe el error. Si es posible, incluye el fragmento que debería revisarse.'],['Envía el reporte por el canal real.','Esta propuesta no incluye un formulario que envíe datos sin una conexión configurada. Puedes contactar con soporte usando el enlace de abajo.']],
 '/terminos':[['Consulta la versión vigente.','El rediseño no sustituye ni reescribe los términos publicados. Las condiciones legales completas permanecen en la página original.'],['Sobre esta demostración.','Los enlaces de recursos abren sitios externos. No se solicitan contraseñas, no se generan respuestas con IA y no se modifica la información de tu cuenta.']],
 '/privacidad':[['Lo que se guarda aquí.','Tus preferencias, las tareas del planificador y las marcas de temas estudiados se guardan en el almacenamiento local de este navegador. No se envían a Universe desde esta demostración.'],['Lo que se conecta fuera.','Al pulsar un recurso externo se abre su proveedor: Universe, Google Drive, YouTube o un canal de la comunidad. Esos servicios aplican sus propias condiciones y políticas de privacidad.']]};out.innerHTML=`<article class="p-panel info-prose">${(content[p]||[]).map(([h,t])=>`<h2>${h}</h2><p>${t}</p>`).join('')}<div class="page-actions">${link('https://universetostudy.com'+p,'Consultar página original','button',true)}${internal('/contacto','Contactar','text-button')}</div></article>`}
