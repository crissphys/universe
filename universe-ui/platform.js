...
const routes={
 ...
 '/examenes':['Aprende de cada examen.','ARCHIVO / CEPREUNI','Conecta evaluaciones, temas y materiales. El archivo distingue los recursos disponibles de los servicios conectados.'],
 ...
 '/terminos':['Las reglas, a la vista.','INFORMACIÓN / TÉRMINOS','Este diseño no modifica las condiciones de uso de Universe. Consulta siempre la versión publicada.'],
 '/privacidad':['Tu información importa.','INFORMACIÓN / PRIVACIDAD','Conoce qué permanece en tu navegador y cuándo estás saliendo hacia otro servicio.'],
};
...
function information(out,p){const content={
 ...
 '/correcciones':[['Cuéntanos dónde ocurre.','Indica el curso, título del material, página o enlace y describe el error. Si es posible, incluye el fragmento que debería revisarse.'],['Envía el reporte por el canal real.','Esta página no incluye un formulario propio de envío. Puedes contactar con soporte usando el enlace de abajo.']],
 '/terminos':[['Consulta la versión vigente.','El rediseño no sustituye ni reescribe los términos publicados. Las condiciones legales completas permanecen en la página original.'],['Cómo funciona esta página.','Los enlaces de recursos abren sitios externos. No se solicitan contraseñas, no se generan respuestas con IA y no se modifica la información de tu cuenta.']],
 '/privacidad':[['Lo que se guarda aquí.','Tus preferencias, las tareas del planificador y las marcas de temas estudiados se guardan en el almacenamiento de este navegador. No se envían a Universe automáticamente.'],['Lo que se conecta fuera.','Al pulsar un recurso externo se abre su proveedor: Universe, Google Drive, YouTube o un canal de la comunidad. Esos servicios aplican sus propias condiciones y políticas de privacidad.']]};out.innerHTML=`<article class="p-panel info-prose">${(content[p]||[]).map(([h,t])=>`<h2>${h}</h2><p>${t}</p>`).join('')}<div class="page-actions">${link('https://universetostudy.com'+p,'Consultar página original','button',true)}${internal('/contacto','Contactar','text-button')}</div></article>`}
